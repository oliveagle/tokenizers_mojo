"""Unigram Tokenizer — top-level orchestrator for SentencePiece-style models.

Pipeline: normalize -> pre_tokenize -> Unigram.encode -> Encoding

Supports loading from HuggingFace tokenizer.json files via from_pretrained_unigram.
"""
...
struct UnigramTokenizer:
    """Tokenizer for Unigram models (SentencePiece-style).

    Attributes:
        normalizer: text normalizer.
        pre_tokenizer: pre-tokenizer (e.g., Metaspace).
        model: Unigram model.
    ...
    def __init__(out self, var model: Unigram):
    ...
    def add_special_token(mut self, token: String) -> Int:
        ...
        if id == -1:
            ...
            self.model.vocab.append(UnigramVocabEntry(token, 0.0))
            self.model.token_to_id[token] = id
    ...
    def _next_vocab_id(self) -> Int:
        ...
        var max_id = -1
        for v in self.model.id_to_token.keys():
            if v > max_id:
                max_id = v
        return max_id + 1
